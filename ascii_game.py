...
class GameState:
    def __init__(self, map_data_strings, npc_definitions):
        ...
        for r, row_list in enumerate(temp_map):
            for c, char_val in enumerate(row_list):
                if char_val in npc_definitions:
                    ...
                    print(f"Initialized NPC ID {new_npc.id} ('{new_npc.char}') at ({new_npc.y},{new_npc.x}), E:{new_npc.energy}, Mode:{new_npc.ai_mode}, Inv:{new_npc.inventory}")
                    npc_id_counter += 1
                    temp_map[r][c] = DEFAULT_WALKABLE_REPLACEMENT_FOR_FEATURES
                elif char_val == REST_SPOT_CHAR:
                    self.rest_spot_locations.append((r,c))
                    print(f"Initialized Rest Spot ('{REST_SPOT_CHAR}') at ({r},{c})")
                    temp_map[r][c] = DEFAULT_WALKABLE_REPLACEMENT_FOR_FEATURES
                elif char_val == RESOURCE_WOOD_CHAR:
                    self.resource_locations["wood"].append((r,c))
                    print(f"Initialized Wood Resource ('{RESOURCE_WOOD_CHAR}') at ({r},{c})")
                    temp_map[r][c] = DEFAULT_WALKABLE_REPLACEMENT_FOR_FEATURES
                elif char_val == RESOURCE_STONE_CHAR:
                    self.resource_locations["stone"].append((r,c))
                    print(f"Initialized Stone Resource ('{RESOURCE_STONE_CHAR}') at ({r},{c})")
                    temp_map[r][c] = DEFAULT_WALKABLE_REPLACEMENT_FOR_FEATURES
        ...
        print(f"Wood Locations: {self.resource_locations['wood']}")
        print(f"Stone Locations: {self.resource_locations['stone']}")
        print("-------------------------------------------")
...
def draw_map(stdscr, game_state, color_pairs):
    ...
    for i, npc_to_display in enumerate(game_state.npcs[:min(curses.LINES - len(game_state.map_data) - 1, 3)]): # Limit to available lines & max 3
        ...
        if status_line_y_pos < curses.LINES -1 :
            ...
            if npc_to_display.target_y is not None:
                ...
                if npc_to_display.current_resource_target_type: # Check if it's not None
                    target_info += f" Res:{npc_to_display.current_resource_target_type[:4]}" # Abbrev resource type

            path_info = f"P:{len(npc_to_display.path)}"
            mode_info = npc_to_display.ai_mode
            # Abbreviate modes for display
            ...
            try:
                stdscr.move(status_line_y_pos, 0) # Move cursor to line start
...
def main(stdscr):
    ...
    while True:
        ...
        if loop_counter % 200 == 0: print(f"\n--- Game Loop Tick {loop_counter} ---") # For text log

        for npc in game_state.npcs:
            current_npc_log_prefix = f"NPC {npc.id} ('{npc.char}' E:{int(npc.energy)} M:{npc.ai_mode[:4]})"
            ...
            if npc.ai_mode == AI_MODE_RESTING:
                ...
                if loop_counter % 20 == 0: print(f"{current_npc_log_prefix} is RESTING. New E: {int(npc.energy)}")
                if npc.energy >= NPC_MAX_ENERGY:
                    npc.ai_mode = AI_MODE_ROAMING; npc.target_y, npc.target_x, npc.path = None, None, []
                    print(f"{current_npc_log_prefix} fully rested, now ROAMING.")

            elif npc.energy < LOW_ENERGY_THRESHOLD and npc.ai_mode != AI_MODE_SEEKING_REST : # Prioritize rest
                 npc.ai_mode = AI_MODE_SEEKING_REST; npc.target_y, npc.target_x, npc.path = None, None, []
                 print(f"{current_npc_log_prefix} low E, now SEEKING_REST.")
                ...
                if npc_is_effectively_idle :
                    ...
                    if not game_state.rest_spot_locations:
                        print(f"{current_npc_log_prefix} wants rest, NO REST SPOTS! Roaming."); npc.ai_mode = AI_MODE_ROAMING
                    else:
                        ...
                        if best_target_spot:
                            ...
                            print(f"{current_npc_log_prefix} SEEKING_REST, NO PATH to any rest spot! Roaming."); npc.ai_mode = AI_MODE_ROAMING
                elif (npc.y, npc.x) == (npc.target_y, npc.target_x) and (npc.y,npc.x) in game_state.rest_spot_locations:
                    npc.ai_mode = AI_MODE_RESTING; npc.path = []
                    print(f"{current_npc_log_prefix} reached rest spot, now RESTING.")

            elif npc.ai_mode == AI_MODE_GATHERING_RESOURCE:
                npc.gathering_timer -= 1
                if loop_counter % 10 == 0: print(f"{current_npc_log_prefix} GATHERING_RESOURCE for {npc.current_resource_target_type}. Timer: {npc.gathering_timer}")
                if npc.gathering_timer <= 0:
                    ...
                    else: print(f"{current_npc_log_prefix} ERR: Invalid res_type {res_type} in gathering.")
                ...
                elif (npc.y, npc.x) == (npc.target_y, npc.target_x):
                    npc.ai_mode = AI_MODE_GATHERING_RESOURCE; npc.gathering_timer = GATHER_TIME_TICKS; npc.path = []
                    print(f"{current_npc_log_prefix} reached {npc.current_resource_target_type} spot, now GATHERING_RESOURCE.")

            elif npc.ai_mode == AI_MODE_ROAMING: # Default roaming if idle and not low energy
                if npc_is_effectively_idle:
                    if random.random() < CHANCE_TO_SEEK_RESOURCE and game_state.resource_locations:
                        chosen_res_type = random.choice(list(game_state.resource_locations.keys()))
                        if game_state.resource_locations.get(chosen_res_type):
                            ...
                            print(f"{current_npc_log_prefix} ROAMING decided to seek resource: {chosen_res_type}.")
                        else: # No spots for chosen type, just roam
                             if all_walkable_coords:
                                ...
                                print(f"{current_npc_log_prefix} ROAMING (no {chosen_res_type} spots), new random T: ({npc.target_y}, {npc.target_x})")
                    elif all_walkable_coords: # Default to random roam
                        pc = [crd for crd in all_walkable_coords if crd != (npc.y, npc.x)]
                        if not pc: pc = all_walkable_coords
                        if pc: npc.target_y, npc.target_x = random.choice(pc)
                        print(f"{current_npc_log_prefix} ROAMING, new random T: ({npc.target_y}, {npc.target_x})")
            ...
            if loop_counter % NPC_MOVE_FREQUENCY == 0:
                if npc.path and npc.ai_mode not in [AI_MODE_RESTING, AI_MODE_GATHERING_RESOURCE]:
                    ...
                    if npc.target_y is not None and (npc.y, npc.x) == (npc.target_y, npc.target_x):
                        ...
                        npc.target_y, npc.target_x, npc.path = None, None, []
                    elif not npc.path and npc.target_y is not None:
                        print(f"{current_npc_log_prefix} path end, not at T. C:({npc.y},{npc.x}) T:({npc.target_y},{npc.target_x}).")
                        npc.target_y, npc.target_x = None, None
