...
class GameState:
    def __init__(self, map_data_strings, npc_definitions):
        ...
        npc_id_counter = 0
        processed_map_rows = []
        # Optional: Print initial setup details once if needed for general debugging
        # print("--- NPC, Resource, Feature Initialization ---")
        ...
        # Removed temporary print of map_data sample from here
...
def draw_map(stdscr, game_state, color_pairs):
    ...
    for r_w, c_w in game_state.water_tile_locations:
        ...
        # Removed temporary log for water animation char from here
    ...
    for i, npc_to_display in enumerate(game_state.npcs[:min(curses.LINES - len(game_state.map_data) - 1, 3)]):
        ...
        if status_line_y_pos < curses.LINES -1 :
            ...
            path_info = f"P:{len(npc_to_display.path)}";mode_info = npc_to_display.ai_mode
            ...
            try: stdscr.move(status_line_y_pos, 0); stdscr.clrtoeol(); stdscr.addstr(status_line_y_pos, 0, status_msg[:curses.COLS-1])
            except curses.error: pass
...
def main(stdscr):
    ...
    curses.init_pair(4, curses.COLOR_YELLOW, curses.COLOR_BLACK); curses.init_pair(5, curses.COLOR_WHITE, curses.COLOR_BLUE)
    curses.init_pair(6, curses.COLOR_RED, curses.COLOR_BLACK); curses.init_pair(7, curses.COLOR_CYAN, curses.COLOR_BLACK)
    curses.init_pair(8, curses.COLOR_MAGENTA, curses.COLOR_BLACK); curses.init_pair(9, curses.COLOR_CYAN, curses.COLOR_BLACK)
    curses.init_pair(10, curses.COLOR_YELLOW, curses.COLOR_DKGRAY); curses.init_pair(11, curses.COLOR_WHITE, curses.COLOR_DKGRAY)
    ...
    if loop_counter == 0: # Print initial setup once, assuming main() is only called once by wrapper
        print("--- Initial GameState Call (from main) ---")
    ...
    for r, row_str in enumerate(game_state.map_data):
        for c, char_val in enumerate(row_str):
            if char_val in game_state.walkable_map_chars_for_roaming:
                all_walkable_coords_for_roaming.append((r,c))

    stdscr.nodelay(True); NPC_MOVE_FREQUENCY = 3
    game_running_duration = 500

    if loop_counter == 0: # Reduce redundant initial prints if main() was ever re-entered
        for npc in game_state.npcs: print(f"Initial: NPC {npc.id} ('{npc.char}') at ({npc.y},{npc.x}), E:{npc.energy}, Mode:{npc.ai_mode}, Patrol:{npc.patrol_path_id}, Inv:{npc.inventory}")
        print(f"Initial Rest Spots: {game_state.rest_spot_locations}")
        print(f"Initial Wood: {game_state.resource_locations['wood']}, Stone: {game_state.resource_locations['stone']}")
        print(f"Initial Patrol Paths: {game_state.patrol_paths}")
        print("--- Starting Main Loop ---")

    while True:
        stdscr.clear(); loop_counter += 1; game_state.game_tick_counter +=1
        if loop_counter > 1 and loop_counter % 600 == 0:
            print(f"\n--- Game Loop Tick {loop_counter} ---")
            for npc_log in game_state.npcs:
                 print(f"Tick {loop_counter}: N{npc_log.id}({npc_log.char} E:{int(npc_log.energy)} M:{npc_log.ai_mode[:7]}) T:({npc_log.target_y},{npc_log.target_x}) P:{len(npc_log.path)} Inv:{npc_log.inventory} Patrol:{npc_log.patrol_path_id} WP:{npc_log.current_patrol_waypoint_index}")
        ...
        for npc in game_state.npcs:
            ...
            if npc.ai_mode == AI_MODE_RESTING:
                ...
                if npc.energy >= NPC_MAX_ENERGY:
                    ...
                    if npc.interrupted_patrol_path_id: npc.ai_mode = AI_MODE_PATROLLING; npc.patrol_path_id = npc.interrupted_patrol_path_id; npc.current_patrol_waypoint_index = npc.interrupted_waypoint_index; npc.interrupted_patrol_path_id, npc.interrupted_waypoint_index = None,0; print(f"{current_npc_log_prefix} rested, resuming PATROL {npc.patrol_path_id} WP{npc.current_patrol_waypoint_index}")
                    elif npc.patrol_path_id: npc.ai_mode = AI_MODE_PATROLLING; print(f"{current_npc_log_prefix} rested, starting PATROL {npc.patrol_path_id}")
                    else: npc.ai_mode = AI_MODE_ROAMING; print(f"{current_npc_log_prefix} rested, now ROAMING.")
            elif npc.energy < LOW_ENERGY_THRESHOLD and npc.ai_mode != AI_MODE_SEEKING_REST :
                interrupted_task = False
                if npc.ai_mode == AI_MODE_PATROLLING and npc.patrol_path_id: npc.interrupted_patrol_path_id = npc.patrol_path_id; npc.interrupted_waypoint_index = npc.current_patrol_waypoint_index; interrupted_task = True; print(f"{current_npc_log_prefix} low E, interrupting PATROL {npc.patrol_path_id} WP{npc.current_patrol_waypoint_index}")
                elif npc.ai_mode in [AI_MODE_SEEKING_RESOURCE, AI_MODE_GATHERING_RESOURCE]: interrupted_task = True; print(f"{current_npc_log_prefix} low E, interrupting RESOURCE task ({npc.current_resource_target_type}).")
                ...
                if interrupted_task: npc.patrol_path_id = None ; print(f"{current_npc_log_prefix} now SEEKING_REST.")
                ...
                if npc.patrol_path_id and (npc_is_effectively_idle or (npc.target_y is not None and (npc.y, npc.x) == (npc.target_y, npc.target_x))):
                    ...
                    if route:
                        if npc.target_y is not None and (npc.y, npc.x) == (npc.target_y, npc.target_x): npc.current_patrol_waypoint_index = (npc.current_patrol_waypoint_index + 1) % len(route); print(f"{current_npc_log_prefix} reached Patrol WP. Next WP idx: {npc.current_patrol_waypoint_index}")
                        npc.target_y, npc.target_x = route[npc.current_patrol_waypoint_index]; npc.path = []
                    else: print(f"{current_npc_log_prefix} invalid patrol_path_id '{npc.patrol_path_id}'. Roaming."); npc.ai_mode = AI_MODE_ROAMING; npc.patrol_path_id = None
                ...
                if npc_is_effectively_idle:
                    ...
                    if not resource_locations_of_type: print(f"{current_npc_log_prefix} wants {npc.current_resource_target_type}, but NO SPOTS! Roaming."); npc.ai_mode = AI_MODE_ROAMING; npc.current_resource_target_type = None
                    else:
                        ...
                        else: print(f"{current_npc_log_prefix} {AI_MODE_SEEKING_RESOURCE}, NO PATH to any {npc.current_resource_target_type}! Roaming."); npc.ai_mode = AI_MODE_ROAMING; npc.current_resource_target_type = None
            elif npc.ai_mode == AI_MODE_ROAMING:
                if npc_is_effectively_idle:
                    if npc.patrol_path_id: npc.ai_mode = AI_MODE_PATROLLING ; print(f"{current_npc_log_prefix} ROAMING, has patrol_id, switching to PATROLLING.")
                        ...
                        if chosen_res_type: npc.ai_mode = AI_MODE_SEEKING_RESOURCE; npc.current_resource_target_type = chosen_res_type; npc.target_y,npc.target_x,npc.path = None,None,[] ; print(f"{current_npc_log_prefix} ROAMING decided to seek resource: {chosen_res_type}.")
            ...
            if npc.target_y is not None and not npc.path and npc.ai_mode not in [AI_MODE_RESTING, AI_MODE_GATHERING_RESOURCE]:
                ...
                if calculated_path: npc.path = calculated_path;
                    ...
                    if npc.ai_mode == AI_MODE_SEEKING_REST: pass
                        ...
                        if npc.patrol_path_id and game_state.patrol_paths.get(npc.patrol_path_id) and game_state.patrol_paths[npc.patrol_path_id]: npc.current_patrol_waypoint_index = (npc.current_patrol_waypoint_index + 1) % len(game_state.patrol_paths[npc.patrol_path_id]) ; print(f"{current_npc_log_prefix} cannot reach patrol WP. Advancing to WP {npc.current_patrol_waypoint_index}.")
            ...
            if loop_counter % NPC_MOVE_FREQUENCY == 0:
                if npc.path and npc.ai_mode not in [AI_MODE_RESTING, AI_MODE_GATHERING_RESOURCE]:
                    ...
                    if npc.target_y is not None and (npc.y, npc.x) == (npc.target_y, npc.target_x):
                        ...
                        elif npc.ai_mode == AI_MODE_PATROLLING: print(f"{current_npc_log_prefix} reached Patrol WP {npc.current_patrol_waypoint_index} at ({npc.y},{npc.x})."); # Target for next WP set by PATROLLING logic
                        else: npc.ai_mode = AI_MODE_ROAMING
                        if npc.ai_mode != AI_MODE_PATROLLING : npc.target_y, npc.target_x, npc.path = None, None, []
